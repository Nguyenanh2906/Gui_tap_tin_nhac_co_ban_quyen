...
# Tạo Flask app
app = Flask(__name__)
app.secret_key = os.environ.get("SESSION_SECRET", "dev-secret-key-change-in-production") #Khóa bí mật bảo vệ session
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)# Đảm bao Flask nhận đúng IP và giao thức khi chạy sau proxy

# Cấu hinh upload folder và các thông số khác
UPLOAD_FOLDER = 'uploads'
ALLOWED_EXTENSIONS = {'mp3'}
MAX_CONTENT_LENGTH = 50 * 1024 * 1024  # Kích thước tệp tối đa 50MB
...
# Định nghĩa models
class Host(db.Model): #Đại diện cho máy chủ nhận file
    __tablename__ = 'hosts'
...
class Sender(db.Model):# Đại diện cho máy gửi file
...
class Transfer(db.Model):#Luu trữ thông tin về các phiên chuyển file
...
class File(db.Model):#Lưu trữ thông tin về các file đã chuyển
    __tablename__ = 'files'
...
class HandshakeRequest(db.Model):#Lưu trữ thông tin về các yêu cầu handshake giữa sender và host
...
def allowed_file(filename):# Kiểm tra định dạng file
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

decoded_files_cache = {} # lưu trữ các file đã giải mã

@app.route('/')
def index():
    return render_template('index.html')

@app.route('/receiver') #Đăng nhập với vai trò là host, kiểm tra IP và tạo host nếu chưa có
def receiver():
...
@app.route('/sender') #Đăng nhập với vai trò là sender, tạo mới nếu chưa có
def sender():
    ip_address = request.remote_addr or '127.0.0.1'
...
#Get: # Hiển thị danh sách các host đã đăng ký, cho phép sender chọn host để gửi file
#Post: # Xử lý việc chọn host, lưu vào session và chuyển hướng đến trang gửi file
def choose_host():
...
@app.route('/send_file', methods=['GET'])#Kiểm tra handskake đã xác nhận chưa,nếu chưa thì báo lỗi hoặc yêu cầu chờ xác thực 
def send_file_get():
...
@app.route('/send_file', methods=['POST']) # Nhận file từ sender, mã hóa và lưu trữ
def send_file_post():
